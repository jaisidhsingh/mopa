...
from models.utils import _assert_no_nan, _round_to_4_decimals
...
class TopkMoE(MoE):
    def __init__(self, config: MoEConfig, experts):
        super().__init__(config, experts)
        self.k = config.k

    @staticmethod
    def _capacity(tokens: int, n_experts: int, k: int, capacity_factor: float) -> int:
